...
$routes_group['Pages']   = get_valores("id","url","paginas_groups","where url not in ('email-marketing') and id in (1,5)");
...
// $routes_group['Servicios'][] ='productos';
// prin($routes_group['Servicios']);

//email_marketing
$routes_group['Email']   = get_valores("id","url","paginas_groups","where url='email-marketing'");
// prin($routes_group['Pages']);
...
if(sizeof($routes_group['Servicios'])>0){
	//all
	$routes_return['/(productos|mas-vistos)$'] 																	='controller=Servicios&method=grid&level=$1';
	//grupo
	...
	//detail
	$routes_return['/producto/(:any)/(:num)$'] 																					='controller=Servicios&method=detail&item=$2';
...
if(sizeof($routes_lists)>0)
	foreach($routes_lists as $rout){
		...
		$routes_return=array_merge($routes_return,[
			...
			'/'.$rout['detail'].'(:any)/(:num)$' 				=> 'controller='.$rout['controller'].'&item=$2',
