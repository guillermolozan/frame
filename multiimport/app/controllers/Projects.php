...
class Projects extends Controller {
	...
	function detail(){
		...
		if($params['item']=='1'){
			$buttons[]=[
			'name'=>'SERVIDORES',
			'sub'=>'Una estrategia de servidores para el éxito ayuda a construir una infraestructura más inteligente.',
			'url'=>'pagina-servidores/2'
			];
		}
		if($params['item']=='4'){
			$buttons[]=[
			'name'=>'REDES',
			'sub'=>'Soluciones en Networking, uniendo calidad con profesionalismo',
			'url'=>'pagina-redes/5'
			];
		}
		...
		if(in_array($params['item'],['1','2']))
			$this->view->assign(['banner'=>'infraestructura_banner_bg.jpg']);
		if(in_array($params['item'],['3']))
			$this->view->assign(['banner'=>'software_banner_bg.jpg']);
		if(in_array($params['item'],['4','5']))
			$this->view->assign(['banner'=>'comunicaciones_banner_bg.jpg']);	
