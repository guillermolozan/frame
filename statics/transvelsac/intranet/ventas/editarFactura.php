...
$TotalVenta = 0;
$sqlDetalle = "SELECT ventasdet.*, unimed.nombre as unimed FROM ventasdet LEFT JOIN unimed ON unimed.idunimed=ventasdet.idunimed WHERE idventascab='$idVentasCab'";
...
$sqlCab = "SELECT ventascab.*, cliente.razonsocial, cliente.ruc, cliente.direccion FROM ventascab LEFT JOIN cliente ON cliente.idcliente=ventascab.idcliente  WHERE idventascab='$idVentasCab'";
...
if ($rcab = leer_registro($consulta))
  {
	 ...
	 $IdCliente = $rcab['idcliente'];
	 $Cliente = $rcab['razonsocial'];
	 $RUC = $rcab['ruc'];
	 $Direccion = $rcab['direccion'];
	 $Monto = $rcab['monto'];
	 $IGV = $rcab['igv'];
	 $Adelanto = $rcab['adelanto'];
	 $TipoVenta = $rcab['tipoventa'];
	 $Cancelado = $rcab['cancelado'];
	 $Saldo = $rcab['saldo'];
	 $codigoComprobante = $IdTipoDoc.'/'.$Serie.'-'.$Numero;
  }
$TotalVenta = $Monto;
...
<FORM ACTION="saveNewFacturaNormal.php"  class="cmxform" id="frmNewFacturaNormal" NAME="frmNewFacturaNormal" METHOD="POST">      
    <table width="98%" align="center" style="margin:0 10px;">
      <tr>
        ...
        <tr>
          ...
          <td class="tdcont" width="40%" colspan="3">&nbsp;
          <input type="text" name="direccion" id="direccion" size="55" readonly="readonly" value="<?php echo $Direccion; ?>"/></td>
    ...
    <td class="tdcont" width="10%">&nbsp;&nbsp;<input type="text" name="comprobante" id="comprobante" maxlength="20" size="20" value="<?php echo $codigoComprobante; ?>" readonly="readonly"/></td>
         	<td class="title" width="8%"><strong>Fecha</strong></td>
          	<td class="tdcont"width="15%">&nbsp;&nbsp;<input type="text" id="datepicker" readonly="readonly" name="fecha" value="<?php echo $Fecha; ?>" size="12"/></td>
        </tr>
        <tr>
      ...
      <td class="tdcont" width="12%">&nbsp;&nbsp;
         <select name="tipoventa" id="tipoventa">
          ...
          <input type="text" name="adelanto" id="adelanto" size="8" value="<?php echo $Adelanto; ?>"/></td>
