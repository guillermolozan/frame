...
<div id="datos">
      ...
      <?php 
			...
			?>
      <div id="busqueda" >
     <form method="post" action="ventas.php" name="frmBuscarVenta" id="frmBuscarVenta" enctype="multipart/form-data" style="font-size:10px; margin-top:10px; text-align:center;">
   ...
   <p class="titulo">Resultado de la Busqueda</p>
	...
	if(isset($_GET['BVenta'])) 
		...
		if(isset($_GET['idCliente'])) 
			{$idProv= $_GET['idCliente'];}
		if(isset($_GET['idComprobante'])) 
			{$codComp= $_GET['idComprobante'];}
	}
	else 
		{$BVenta='0';}
...
if($buscarVenta=='1' or $BVenta=='1')	
{
	...
	if(isset($codComprobante) and $codComprobante != "")
	{
		if($filtro != ""){
			$filtro .= " " . "AND";
			}
		$filtro = " " . "c.idComprobante LIKE '%$codComp%'";
		$cadena4="idComprobante=".$idComprobante;
	}
  }
	if($filtro != ""){
		$filtro = " " . "WHERE" . $filtro;
  ...
  if($buscarVenta=='1' or $BVenta=='1') {
	...
	if($total_registros) {
		...
		$principal="ventas";//pagina principal
		if(($pagina - 1) > 0) {
