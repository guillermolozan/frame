...
<br /><br />
   ...
   <?php 
	...
	if($buscarCliente=='1' or $BCliente=='1')	
	{
	    if(isset($data) and $data != "")
    	{
			...
			$filtro = $filtro . " " . "nomCliente LIKE '%$data%'";
			$cadena2="data=".$data;
	    }
    }
  ...
  if($buscarCliente=='1' or $BCliente=='1') {
	...
	if($total_registros) {
		...
		$principal="searchClientesCaja";//pagina principal
		if(($pagina - 1) > 0) {
   ...
   else
  {
	...
	?>
  		...
  		<tr>
            ...
            <td align="center" class="title"><div align="center">Cod. Cliente</div></td>
...
window.opener.document.getElementById('idcliprov').value ='<?php echo $rs['idcliente']; ?>';
window.opener.document.getElementById('rucdni').value ='<?php echo $rs['ruc']; ?>';
window.opener.document.getElementById('razonsocial').value ='<?php echo $rs['razonsocial']; ?>';
window.close();
">Agregar</a></td>
        </tr>
    <?php		
