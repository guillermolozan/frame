...
if($this_me){
	...
	if($objeto_tabla[$this_me]['page']==1){	$filtrar_page2=1;	$_SESSION['page']=($_SESSION['page']!='' and in_array($_SESSION['page'],$IdPageS))?$_SESSION['page']:$IdPageS[0];	}
	if($objeto_tabla[$this_me]['web']==1){	$filtrar_web2=1;	$_SESSION['web']=($_SESSION['web']!=''  and in_array($_SESSION['page'],$IdWebS))?$_SESSION['web']:$IdWebS[0];	}	
...
.foot a { margin-right:5px; text-decoration:none; }
.foot a:hover { text-decoration:underline; }
.bloque_titulo { 
background-color:#<?php echo $vars['GENERAL']['BG_COLOR_3'];?>;
<?php /*echo css_gradient_vertical("0%","#".$BG_COLOR_3_OPP,"250%","#".$vars['GENERAL']['BG_COLOR_3']);*/?>
<?php echo css_gradient_vertical("0%","#FFFFFF","250%","#eeeeee");?>
<?php echo "border:1px solid #cccccc; border-bottom:7px solid #".$vars['GENERAL']['BG_COLOR_3'].";";?>
}
<?php 

foreach($objeto_tabla as $ot){ if($ot['archivo'].".php"==$SERVER['ARCHIVO']){ $oott=$ot; } }
if(sizeof($oott['campos'])>9){ ?>.formulario { <?php echo "border-bottom:7px solid #".$vars['GENERAL']['BG_COLOR_3'].";";?>}<?php } 
?>
#inner { <?php echo "border-bottom:7px solid #".$vars['GENERAL']['BG_COLOR_3'].";";?>}
...
if($_GET['block']=='form'){
?><style>#inner {height:290px;overflow: auto;}.formulario {border:0;}</style><?php 	
}
if($_GET['block']!='form'){ 
...
if($_COOKIE['admin']=='1'){
	...
	$menus_d['salir_admin'] =  "<a href='maquina.php?accion=borrarcookie'></span>SALIR DE MASTER</a>";
...
if($Open or $Local==1){
	...
	foreach($usuars as $usu){ if($usu['nombre']!=''){
	
	foreach($tables_usuarios as $tu){	$FiL=select("usuarios_acceso_nombre as nombre,usuarios_acceso_password as password",$tu['tabla'],"where id_sesion='".$usu['id']."'",0); if(sizeof($FiL)>0){ $FiLa=$FiL['0']; continue; }	}
...
//	$tabla_usuarios_id_sub
	...
	$mmmM = "<li class='menudown m_user' ><a href='custom/usuarios_acceso.php'>";
...
?><a href="./" class="link_header" ><?php echo str_replace("Panel de Administración","Sistema",$html_titulo);?></a><?php
?></div><?php 
/*if($Local and !$LOGIN){ 
if($Open){ 
...
foreach($ItemsPAGES as $ip){ echo "<li ".(($ip['id']==$_SESSION['page'])?'class="selected"':'')."><a href='actions.php?accion=changepage&id=".$ip['id']."'>".$ip['nombre']."</a>
</li>";}
echo '</ul>';
}
...
foreach($ItemsWEBS as $ip){ echo "<li ".(($ip['id']==$_SESSION['web'])?'class="selected"':'')."><a href='actions.php?accion=changeweb&id=".$ip['id']."'>".$ip['nombre']."</a>
</li>";}
echo '</ul>';
}
/////////////////////
