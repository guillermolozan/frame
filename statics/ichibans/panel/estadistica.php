	...
	if($can['queries']=='1'){
		$qus[]=$can;
			...
			$html_filter.= '<li>
					...
					<label for="fs_alll">General</label>
					</li>';			
			foreach($qus as $querie){ 
				if($querie['tipo']=='hid' and ($querie['opciones'])){ 
					...
					<label for="fs_'.$querie['campo'].'"><b>por</b> '.$querie['label'].'</label>
					</li>';
...
?>	
<div class='filters' style='width:100%;'><div style='padding:1px 0  0 20px;'><?php echo $html_filter_fecha; ?></div></div>
...
<object width="550" height="380" type="application/x-shockwave-flash" data="<?php echo $DIR;?>js/open-flash-chart.swf" id="my_chart" style="visibility: visible;">
<param name="flashvars" value="data-file=<?php echo $DIR;?>load_estadistica.php?b=<?php echo $_GET['OB'];?>||<?php echo $terfilSTFECHA;?>||">
</object>
...
?></ul>
<style>.bloque_content_stat{display:block;}</style><?php
if($_GET['ran']!=''){
