		...
		*/
		...
		if($_SERVER['REQUEST_METHOD']=='POST'){
			...
			$sended=$email->send(
				...
				implode(',',$this->view->vars['web_email_admin']),
				"Mensaje de Cotizacion del producto #".$post['codigo'],
				[
				...
				[
					'name'		 =>'cotización para administrador'
				]
			);

			$sended_response=$email->send(
				$_POST['email'],
				"Cotizacion del producto #".$post['codigo'],
				[
					'name_right' =>$this->view->vars['web_name'],
					'title'      =>"Cotización",
					// 'subtitle'   =>'Se recibió un mensaje desde la web '.$this->view->vars['web_name'],
				...
				[
					'name'		 =>'cotización para usuario'
				]
			);

			if($sended){	$this->setMessage($email,"Mensaje Enviado");		} 
			// else { echo 'noooo'; }
