...
function render_indicadores_economicos($array){

	$html ="";
	
	$html.= '<div class="bloque_tabla" style="clear:both;">';
...
//		$html.="</div>";
		...
		foreach($array as $i=>$Fila){
			if($i!=0){
			$html.="<tr class='".(($i%2==1)?"par":"impar")."'>";

			foreach($Fila as $j=>$Cell){
				if($j==0){
				$html.='<th class="izq">'; $html.=$Cell;	$html.="</th>";					
				}else{
				$html.="<td>"; $html.=$Cell;	$html.="</td>";	
				}
			}
			$html.="</tr>";
			}
