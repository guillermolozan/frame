...
elseif(!empty($_GET['fil']) and !empty($_GET['val'])) {
	switch($_GET['fil']){
		...
		$TITULO = Titulo_Filtro("Agenda del mes de ",$_GET['val']);
		$PORPAG = 10;	
...
$LISTA = paginacion(
			...
			,array(
				...
				'foto:atributos'=>array('atributos'=>'bloage_imas,{fecha_creacion},{file},2,150x110,1'),
...
$LISTA['combo']=array(
			...
			'items'=>select(
				...
				,0
				,array(
				'opcion'=>array('function'=>array('global $Array_Meses; list($yy,$mm)=explode("-","{value}"); echo "Agenda del mes de ".$Array_Meses[$mm]." de ".$yy;')),
