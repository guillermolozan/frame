...
$gru=array();
	...
	$gru['menu']=select(
			...
			0,
				array(
				    'url'=>"index.php?".$filtro_param."modulo=items&tab=clases_items&acc=file&id={id}&friendly={nombre}",							
					'nivel'=>array('match'=>array('{id_subgrupo}','','menu_nivel_2','menu_nivel_2')),
					'class'=>array('match'=>array('{id}',$ID,'selected','')),
				)
			);

	$gru['header']['nombre']='Clases Personalizadas';
