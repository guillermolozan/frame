...
if($MASTERBLOCK['header_after']){ ?>
    <div class="div_fila" >    
        ...
        <?php
        foreach($COMMON['header_after'] as $gm){ 
        ...
        <?php foreach($COMMON['header_after_fotos'] as $item){ 
        ?><div id="son_menu2_main_<?php echo $item['id_grupo'];?>_<?php echo $item['id']; ?>" class="div_fila_overflow <?php 
        web_selector_control($SELECTED,$THIS.'2fotos',"listados,bloques",1);
        ?>">
        <div class="div_inner">
	...
	<script>
        window.addEvent('domready', function() {
            $$('.id_header_after li.li').each(function(ee){	 over_menu({e:ee,x:'left',y:'bottom',rel:'element'}); });            
            $$('.id_header_after2 li.li').each(function(ee){ over_menu({e:ee,x:'left',y:'bottom',rel:'element'}); });       
        });	
    </script>    
